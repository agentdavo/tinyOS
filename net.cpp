 ...
 * @details
 * Legacy/demo networking implementation.
 *
 * This file is intentionally not part of the controller build. It remains here
 * only to avoid breaking older DSP/demo code that still includes `net.hpp`.
 * Controller networking now lives in the HAL NIC drivers plus:
 * - EtherCAT masters on `eth1` / `eth2`
 * - raw HMI service on `eth0`
 *
 * C++20 features:
