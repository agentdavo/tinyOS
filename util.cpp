...
bool str_to_float(std::string_view input, float& out_val) noexcept {
    ...
    if (i < input.size() && (input[i] == 'e' || input[i] == 'E')) {
        ...
        for (int k = 0; k < abs_exp; ++k) {
            pow10 *= 10.0;
            if (pow10 > static_cast<double>(std::numeric_limits<float>::max())) return false;
        }
    ...
    constexpr double float_max_d = static_cast<double>(std::numeric_limits<float>::max());
    if (result != result || result > float_max_d || result < -float_max_d) {
        return false;
    }
...
int k_vsnprintf(char* buffer, size_t bufsz, const char* format, va_list args) noexcept {
    ...
    while (*format && buf_ptr < buf_write_end) {
        if (*format == '%') {
            format++;
            // Parse flags. Honour '-' (left-align) and '0' (zero-pad). Others
            ...
            if (format[0] == 'l' && format[1] == 'l') {
                ...
                // size_t / ssize_t — 64-bit on aarch64 LP64.
                is_size_t = true; format++;
            } else if (format[0] == 'l') {
                // `long` on aarch64 LP64 is 64-bit; treat %lu/%ld like %llu/%lld.
                is_long_long = true; format++;
            }
            ...
            switch (*format) {
                ...
                case 'u': {
                    if (is_long_long || is_size_t) current_segment_len = uint64_to_str(va_arg(args, unsigned long long), temp_num_buf, sizeof(temp_num_buf));
                ...
                case 'x': case 'X': case 'p': {
                    ...
                    else if (is_long_long || is_size_t) hex_val = va_arg(args, unsigned long long);
            ...
            if (str_to_copy_from && current_segment_len > 0) {
                int pad = width - current_segment_len;
                if (pad < 0) pad = 0;
                const char pad_char = zero_pad ? '0' : ' ';
                auto write_pad = [&]() {
                    for (int k = 0; k < pad && buf_ptr < buf_write_end; ++k) {
                        *buf_ptr++ = pad_char;
                        total_written_chars++;
                    }
                };
                if (!left_align) write_pad();
