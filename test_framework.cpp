 ...
 * results via UART. Generated for v1.7 to meet v1.7 testing requirements, with improved error
...
private:
    ...
    static bool test_ui_builder_parse_widget(kernel::hal::UARTDriverOps* uart_ops) {
        assert(uart_ops);
        constexpr const char* record =
            "button\tid=start_btn\ttext=Cycle Start\tx=12\ty=24\tw=180\th=48\tcolor=#112233\tbg=#445566\taction=demo:cycle\tfocus=7\n";
        const ui_builder::WidgetSpec spec = ui_builder::parse_widget(record, std::strlen(record));
        if (spec.type != ui_builder::WidgetType::Button) {
            uart_ops->puts("Expected button widget type\n");
            return false;
        }
        if (std::string_view(spec.id) != "start_btn" || std::string_view(spec.text) != "Cycle Start") {
            uart_ops->puts("Widget id/text parse failed\n");
            return false;
        }
        if (spec.x != 12 || spec.y != 24 || spec.w != 180 || spec.h != 48 || spec.focus_order != 7) {
            uart_ops->puts("Widget geometry/focus parse failed\n");
            return false;
        }
        if (std::string_view(spec.action) != "demo:cycle" ||
            spec.color != 0xFF112233U || spec.bg_color != 0xFF445566U) {
            uart_ops->puts("Widget action/color parse failed\n");
            return false;
        }
        return true;
    }

    static bool test_ui_builder_include_and_validation(kernel::hal::UARTDriverOps* uart_ops) {
        assert(uart_ops);
        constexpr const char* valid_tsv =
            "page\tid=template\ttitle=Template\n"
            "panel\tid=template_panel\tx=0\ty=0\tw=320\th=180\tlayout=vertical\n"
            "label\tid=template_title\ttext=Template\n"
            "button\tid=template_button\ttext=Cycle\taction=demo:cycle\n"
            "page\tid=main\ttitle=Main\n"
            "panel\tid=main_panel\tx=0\ty=0\tw=320\th=200\tlayout=vertical\n"
            "include\tpage=template\n"
            "child\tparent=main_panel\twidget=main__template_title\talign=center\n"
            "child\tparent=main_panel\twidget=main__template_button\talign=right\n"
            "action\twidget=main__template_button\tevent=click\ttarget=goto:template\n";
        if (!ui_builder::load_tsv(valid_tsv, std::strlen(valid_tsv))) {
            uart_ops->puts("Expected valid TSV to load\n");
            uart_ops->puts(ui_builder::last_error());
            uart_ops->puts("\n");
            return false;
        }
        if (!ui_builder::set_page("main") || std::string_view(ui_builder::active_page_id()) != "main") {
            uart_ops->puts("Failed to activate main page\n");
            return false;
        }
        if (!ui_builder::set_page("template") || std::string_view(ui_builder::active_page_id()) != "template") {
            uart_ops->puts("Failed to activate template page\n");
            return false;
        }

        constexpr const char* invalid_tsv =
            "page\tid=main\n"
            "panel\tid=root\tx=0\ty=0\tw=100\th=100\n"
            "child\tparent=root\twidget=missing\n";
        if (ui_builder::load_tsv(invalid_tsv, std::strlen(invalid_tsv))) {
            uart_ops->puts("Expected invalid TSV to fail\n");
            return false;
        }
        if (std::string_view(ui_builder::last_error()) != "child record references unknown widget" ||
            ui_builder::last_error_line() != 3U) {
            uart_ops->puts("Unexpected builder validation error surface\n");
            return false;
        }
        return true;
    }
...
void register_tests() {
    ...
    tf.register_test({"ui_builder_parse", test_ui_builder_parse_widget, "Test UI builder widget parsing"});
    tf.register_test({"ui_builder_load", test_ui_builder_include_and_validation, "Test UI builder include and validation"});
}
