...
extern "C" void hal_irq_handler(uint32_t core_id) {
    ...
    if (irq_id < 1020) { 
        if (irq_id == kernel::hal::SYSTEM_TIMER_IRQ) { // SYSTEM_TIMER_IRQ is 30 (PPI)
        ...
        } else if (irq_id == ::hal::qemu_virt_arm64::IRQ_UART0) { // This is SPI 33
            // UART specific handling if interrupt-driven
        } else if (irq_id == ::hal::qemu_virt_arm64::IRQ_VIRTIO_NET) { // This is SPI 35
            // VirtIO Net specific handling
        }
        // Add other IRQ handlers here based on their SPI/PPI ID
        irq_ops->end_irq(core_id, irq_id);
    } else if (irq_id == 1023) {
        // Spurious interrupt
        // For spurious IRQs, some GIC controllers do not require/want EOI.
        // However, acking it (which returns 1023) is usually enough.
        // If issues arise, one might consider not calling end_irq for 1023.
    }
}
