 ...
 * @details
 * Implements the ARM64 HAL for QEMU virt, providing platform-specific operations for UART,
 * interrupts (GICv2), timers, DMA, I2S, networking (virtio-net), GPIO, power, and watchdog.
 ...
 *
 * @version 1.7
 * @see hal_qemu_arm64.hpp, miniOS.hpp, dsp.hpp
 */
...
void UARTDriver::putc(char c) {
    while (read_reg(0x18) & 0x20); // Wait for TX not full
...
char UARTDriver::getc_blocking() {
    while (!(read_reg(0x18) & 0x10)); // Wait for RX not empty
    return static_cast<char>(read_reg(0x00));
}

// --- IRQController ---
void IRQController::write_gicd(uint32_t offset, uint32_t value) {
    write_mmio(GIC_DISTRIBUTOR_BASE + offset, value);
}

uint32_t IRQController::read_gicd(uint32_t offset) {
    return read_mmio(GIC_DISTRIBUTOR_BASE + offset);
}

void IRQController::write_gicc(uint32_t offset, uint32_t value) {
    write_mmio(GIC_CPU_INTERFACE_BASE + offset, value);
}

uint32_t IRQController::read_gicc(uint32_t offset) {
    return read_mmio(GIC_CPU_INTERFACE_BASE + offset);
}

void IRQController::enable_core_irqs(uint32_t core_id, uint32_t source) {
    write_gicc(0x00, 1); // Enable CPU interface
    enable_irq_line(source);
}

void IRQController::disable_core_irqs(uint32_t core_id) {
    write_gicc(0x00, 0); // Disable CPU interface
}

void IRQController::init_distributor() {
    write_gicd(0x000, 0); // Disable distributor
    for (uint32_t i = 32; i < 256; i += 32) {
        write_gicd(0x180 + (i / 8), 0xFFFFFFFF); // Clear pending
        write_gicd(0x280 + (i / 8), 0xFFFFFFFF); // Clear active
    }
    write_gicd(0x000, 1); // Enable distributor
}

void IRQController::init_cpu_interface() {
    write_gicc(0x04, 0x1E); // Priority mask
    write_gicc(0x08, 0); // Binary point
    write_gicc(0x00, 1); // Enable CPU interface
}

uint32_t IRQController::ack_irq(uint32_t core_id) {
    return read_gicc(0x0C); // Acknowledge interrupt
}

void IRQController::end_irq(uint32_t core_id, uint32_t source) {
    write_gicc(0x10, source); // End interrupt
}

void IRQController::enable_irq_line(uint32_t source) {
    write_gicd(0x100 + (source / 32) * 4, 1U << (source % 32));
}

void IRQController::set_irq_priority(uint32_t source, uint8_t priority) {
    write_gicd(0x400 + (source / 4) * 4, priority << ((source % 4) * 8));
}
...
void TimerDriver::init_core_timer_interrupt() {
    write_mmio(0x40000040, 1); // Enable CNTP
    write_mmio(0x4000002C, 0xFFFFFFFF); // Set timer compare
}

void TimerDriver::ack_core_timer_interrupt() {
    write_mmio(0x40000024, 0); // Clear interrupt
}
...
uint64_t TimerDriver::get_system_time_us() {
    ...
    return (cntpct * 1000000) / timer_freq_hz_;
}
...
void TimerDriver::hardware_timer_irq_fired(uint32_t core_id) {
    ...
    write_mmio(0x4000002C, 0xFFFFFFFF); // Reset compare
}
...
bool WatchdogDriver::start_watchdog(uint32_t timeout_ms) {
    write_mmio(RTC_BASE + 0x08, timeout_ms * 1000); // Timeout in us
    write_mmio(RTC_BASE, 1); // Enable
    return true;
}

bool WatchdogDriver::reset_watchdog() {
    write_mmio(RTC_BASE + 0x04, 1); // Reset
    return true;
}
...
void PlatformQEMUVirtARM64::early_init() {
    uart_driver_.puts("[miniOS v1.7] ARM64 QEMU Virt Early Init\n");
    irq_controller_.init_distributor();
}

void PlatformQEMUVirtARM64::core_early_init() {
    irq_controller_.init_cpu_interface();
    timer_driver_.init_core_timer_interrupt();
}

void PlatformQEMUVirtARM64::panic(const char* msg, const char* file, int line) {
